...
class MainTest extends FunSuite {
    ...
    val response = requests.post(
      ...
      data = ujson.Obj(
        "requestCarrier" -> ujson.Obj(
          "name"         -> "John express",
          "workingRange" -> ujson.Obj("start" -> "09:00", "end" -> "18:00"),
          "workingArea" -> ujson.Obj(
            "point"  -> ujson.Obj("latitude" -> 43.2969901, "longitude" -> 5.3789783),
            "radius" -> 42
          ),
          "maxWeight"       -> 50,
          "maxVolume"       -> 40,
  ...
  test("return Conflict status and reason when carrier already exists") {
    val json = ujson.Obj(
      "requestCarrier" -> ujson.Obj(
        "name"         -> "Super Delivery",
        "workingRange" -> ujson.Obj("start" -> "09:00", "end" -> "18:00"),
